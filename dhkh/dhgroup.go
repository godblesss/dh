...
func (self *DHGroup) GeneratePrivateKey(randReader io.Reader) (key *DHKey, err error) {
	...
	half := self.p.Div(self.p, new(big.Int).SetInt64(2))
	x, err := rand.Int(randReader, half)
	if err != nil {
		return
	}
	x = x.Add(x, half)
...
func (self *DHGroup) ComputeKey(pubkey *DHKey, privkey *DHKey) (key *DHKey, err error) {
	...
	if pubkey.y.Sign() <= 0 {
		err = errors.New("DH parameter out of bounds1")
		return
	}
	if pubkey.y.Cmp(self.p) >= 0 {
		err = errors.New("DH parameter out of bounds2")
	}
	if privkey.x == nil {
